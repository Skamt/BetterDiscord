...
export default class Patcher {
	 ...
	 */
	static before<M extends object, K extends Extract<keyof M, string>>(caller: string, moduleToPatch: M, functionName: K, callback: M[K] extends (...a: any[]) => any ? BeforeCallback<M[K]> : never, options: PatchOptions = {}) {
	 ...
	 * @return {module:Patcher~unpatch} Function with no arguments and no return value that should be called to cancel (unpatch) this patch. You should save and run it when your plugin is stopped.
	 */
	static after<M extends object, K extends Extract<keyof M, string>>(caller: string, moduleToPatch: M, functionName: K, callback: M[K] extends (...a: any[]) => any ? AfterCallback<M[K]> : never, options: PatchOptions = {}) {
	 ...
	 * @return {module:Patcher~unpatch} Function with no arguments and no return value that should be called to cancel (unpatch) this patch. You should save and run it when your plugin is stopped.
	 */
	static instead<M extends object, K extends Extract<keyof M, string>>(caller: string, moduleToPatch: M, functionName: K, callback: M[K] extends (...a: any[]) => any ? InsteadCallback<M[K]> : never, options: PatchOptions = {}) {
		...
		const child: GenericChildPatch = {
			...
			unpatch: () => {
				patch.children.splice(
					patch.children.findIndex(cpatch => cpatch.id === child.id && cpatch.type === type),
					1
				);
