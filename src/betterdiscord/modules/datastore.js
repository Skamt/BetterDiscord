...
export default new class DataStore {
	...
	get pluginFolder() {
		return this._pluginFolder || (this._pluginFolder = path.resolve(Config.dataPath, "plugins"));
	}
	get themeFolder() {
		return this._themeFolder || (this._themeFolder = path.resolve(Config.dataPath, "themes"));
	}
	get customCSS() {
		return this._customCSS || (this._customCSS = path.resolve(this.dataFolder, "custom.css"));
	}
	get baseFolder() {
		return this._baseFolder || (this._baseFolder = path.resolve(Config.dataPath, "data"));
	}
	get dataFolder() {
		return this._dataFolder || (this._dataFolder = path.resolve(this.baseFolder, `${releaseChannel}`));
	}
	get webpackCache() {
		return this._webpackCache || (this._webpackCache = path.resolve(this.dataFolder, "webpackCache.json"));
	}
	getPluginFile(pluginName) {
		return path.resolve(Config.dataPath, "plugins", pluginName + ".config.json");
	}
	...
	getWebpackCache() {
		if (fs.existsSync(this.webpackCache)) return JSON.parse(fs.readFileSync(this.webpackCache));
		return {};
	}

	setWebpackCache(cache) {
		if (this.setWPCTimeout) clearTimeout(this.setWPCTimeout);
		// debounce to only apply after 100ms of inactivity (using Utilities causes a circular reference and webpack explodes)
		this.setWPCTimeout = setTimeout(() => {
