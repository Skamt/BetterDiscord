...
class InstallCSS {
			...
			codeActions.children = [
				...
				<PackageOpenIcon
					...
					onClick={async event => {
						...
						Modals.showConfirmationModal(t("Modals.confirmAction"), t("Modals.installCss"), {
	...
	static handleCSSInstall(cssBlock: string) {
		try {
			...
			InstallCSS.activeNotifications.set(notificationId, cssBlock);
			...
			NotificationUI.show({
				...
				actions: [
					{
						label: "Keep",
						onClick: () => InstallCSS.keepChanges(notificationId)
					}
				],
				onClose: () => InstallCSS.revertCSS(notificationId)
			});
		} catch (error) {
	...
	static keepChanges(notificationId: string) {
		InstallCSS.activeNotifications.delete(notificationId);
		Toasts.show(t("CustomCSS.cssKept"), { type: "success" });
	}

	static revertCSS(notificationId: string) {
		const cssBlock = InstallCSS.activeNotifications.get(notificationId);
		if (!cssBlock) return;
