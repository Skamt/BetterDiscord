					...
					function PanelHeader() {
						...
						return (
							<>
								...
								{node && ReactDOM.createPortal(<div className="bd-settings-page-title-children">{ref.current.children}</div>, node)}
							</>
						);
					}
		...
		Patcher.after("SettingsManager", search, "search", (that, args, res) => {
			...
			for (const collection of Settings.collections) {
				...
				insert(collection.id, {
					...
					searchableTitles: ["betterdiscord", collection.name, ...items]
				});
			}
			...
			for (const panel of Settings.panels.sort((a, b) => (a.order > b.order ? 1 : -1))) {
				const content = {
					label: panel.label,
					searchableTitles: ["betterdiscord", panel.label, typeof panel.searchable === "function" ? panel.searchable().filter(m => typeof m === "string") : []].flat()
				};
