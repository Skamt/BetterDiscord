...
import electron, { ipcMain as ipc, type BrowserWindow } from "electron";
...
if (process.platform === "win32" || process.platform === "darwin")
	bdFolder = path.join(electron.app.getPath("userData"), "..");
else
	bdFolder = process.env.XDG_CONFIG_HOME
		? process.env.XDG_CONFIG_HOME
		: path.join(process.env.HOME!, ".config"); // This will help with snap packages eventually
...
export default class BetterDiscord {
	...
	static getSetting(category: string, key: string) {
		...
		try {
			...
			// eslint-disable-next-line @typescript-eslint/no-require-imports
			BetterDiscord._settings = require(settingsFile) ?? {};
			return BetterDiscord._settings[category]?.[key];
		} catch {
	...
	static ensureDirectories() {
		...
		if (!fs.existsSync(path.join(dataFolder, "stable")))
			fs.mkdirSync(path.join(dataFolder, "stable"));
		if (!fs.existsSync(path.join(dataFolder, "canary")))
			fs.mkdirSync(path.join(dataFolder, "canary"));
		if (!fs.existsSync(path.join(dataFolder, "ptb")))
			fs.mkdirSync(path.join(dataFolder, "ptb"));
		if (!fs.existsSync(path.join(dataFolder, "development")))
			fs.mkdirSync(path.join(dataFolder, "development"));
		if (!fs.existsSync(path.join(bdFolder, "plugins")))
			fs.mkdirSync(path.join(bdFolder, "plugins"));
		if (!fs.existsSync(path.join(bdFolder, "themes")))
			fs.mkdirSync(path.join(bdFolder, "themes"));
	}
	...
	static setup(browserWindow: BrowserWindow) {
		...
		try {
			...
			process.env.DISCORD_RELEASE_CHANNEL =
				require(buildInfoFile).releaseChannel;
		} catch {
		...
		if (electron.app.setAsDefaultProtocolClient("betterdiscord")) {
			...
			const protocol = process.argv.find((arg) =>
				arg.startsWith("betterdiscord://"),
			);
			if (protocol) {
	...
	static disableMediaKeys() {
		...
		const originalDisable =
			electron.app.commandLine.getSwitchValue("disable-features") || "";
